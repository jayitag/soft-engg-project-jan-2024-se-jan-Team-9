    ...
    >
      ...
      <b-row class="row ticket-card-buttons">
        <b-col class="col text-right">
          <b-button v-if="status==='resolved' && !(is_faq==='True')" variant="link" @click="suggestForFAQ">Suggest for FAQ</b-button>
          <div v-if="status==='resolved'  && (is_faq==='True')">This ticket is already marked as an FAQ</div>
          <b-button variant="link" @click="reply"><b-icon icon="reply" aria-hidden="true"></b-icon>Reply</b-button>
           <b-dropdown variant="link" text="Forward" right>
          <b-dropdown-item
            v-for="(option, index) in forwardOptions"
            :key="index"
            @click="forwardTicket(index)"
          >
            {{ option }}
          </b-dropdown-item>
        </b-dropdown>
          <b-button ref="flagButton" variant="link" @click="toggleFlag"> <b-icon :icon="is_flag ? 'flag-fill' : 'flag'" class="mr-1"></b-icon>
            Flag</b-button>
